document.addEventListener("DOMContentLoaded", function () {
  ...
  var percent = document.getElementById("quiz-position__percent-change");
  const answersObj = {
    step0: {
      question: "",
      answers: [],
  ...
  btnsNext.forEach((btn, btnIndex) => {
    btn.addEventListener("click", (event) => {
      ...
      percent.innerText = Number(percent.textContent) + 25;
  ...
  const sendForm = () => {
    form.addEventListener("submit", (event) => {
      ...
      if (document.getElementById("quiz-policy").checked) {
        ...
        alert("Give consent to the processing of personal data");
      }
    });
  };
